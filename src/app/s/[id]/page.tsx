...
export default async function SpecialistPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="container" style={{ padding: "24px 20px 0" }}>
      <Link href="/" className="link" style={{ fontSize: "0.9rem" }}>← Каталог</Link>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "minmax(0, 1fr) 360px", gap: 28, marginTop: 16, alignItems: "start" }} className="profile-grid">
        ...
        <div style={{ minWidth: 0 }}>
          ...
          <div style={{ display: "flex", gap: 16, alignItems: "center", marginBottom: 18 }}>
            ...
            <div style={{ minWidth: 0 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
                ...
                {s.verified && <span className="badge badge-verified">✔ Проверен</span>}
                <span style={{ display: "inline-flex", alignItems: "center", gap: 5 }}>
                  <Stars rating={s.rating} size={15} />
                  <span className="muted" style={{ fontSize: "0.85rem" }}>
                    {s.review_count > 0 ? `${s.review_count} отзыв(ов)` : "нет отзывов"}
                  </span>
                </span>
              </div>
              <div className="soft" style={{ marginTop: 4 }}>
                <span className="badge badge-soft" style={{ marginRight: 8 }}>{p?.emoji} {p?.label ?? s.profession}</span>
                📍 {s.city} · опыт {s.experience_years} лет
              </div>
            </div>
          ...
          <div style={{ marginTop: 22 }}>
            <h3 className="h2" style={{ fontSize: "1.15rem", marginBottom: 10 }}>🎬 Видео-визитка</h3>
            {s.video_url ? (
              <VideoCard url={s.video_url} />
            ) : (
              <div className="card card-pad" style={{ textAlign: "center", color: "var(--text-mute)" }}>
                Видео-визитка пока не добавлена.
              </div>
            )}
          </div>
          ...
          {(() => {
            const rows = fieldsFor(s.profession)
              .map((f) => ({ label: f.label, val: formatAttr(f, s.attributes?.[f.key]) }))
              .filter((r) => r.val);
            if (rows.length === 0) return null;
            return (
              <div style={{ marginTop: 20 }}>
                <h3 className="h2" style={{ fontSize: "1.15rem", marginBottom: 10 }}>Характеристики</h3>
          ...
          <div style={{ marginTop: 22 }}>
            <h3 className="h2" style={{ fontSize: "1.15rem", marginBottom: 8 }}>О специалисте</h3>
          ...
          {s.tags.length > 0 && (
            <div style={{ display: "flex", flexWrap: "wrap", gap: 7, marginTop: 14 }}>
              {s.tags.map((t) => (
                <span key={t} className="chip" style={{ cursor: "default", fontSize: "0.82rem" }}>#{t}</span>
              ))}
            </div>
          )}
          ...
          {publicSocials.length > 0 && (
            <div style={{ marginTop: 22 }}>
              <h3 className="h2" style={{ fontSize: "1.15rem", marginBottom: 10 }}>Соцсети</h3>
              <SocialLinks socials={publicSocials} />
            </div>
          )}

          <div style={{ marginTop: 22 }}>
            <h3 className="h2" style={{ fontSize: "1.15rem", marginBottom: 10 }}>📅 Занятость</h3>
            {busy.length === 0 ? (
              <div className="card card-pad" style={{ color: "var(--good)", display: "flex", alignItems: "center", gap: 8 }}>
                ✓ Свободных дат много — уточните нужный день в запросе.
              </div>
            ) : (
              <div className="card card-pad">
                <p className="soft" style={{ margin: "0 0 10px", fontSize: "0.9rem" }}>Ближайшие занятые дни:</p>
          ...
          <div style={{ marginTop: 26 }}>
            <h3 className="h2" style={{ fontSize: "1.15rem", marginBottom: 12 }}>
              Отзывы {s.review_count > 0 && <span className="muted" style={{ fontWeight: 400 }}>· {s.rating.toFixed(1)} ★</span>}
            </h3>
