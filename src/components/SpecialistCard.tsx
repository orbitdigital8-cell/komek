  ...
  return (
    <Link href={`/s/${s.id}`} className="card lift" style={{ overflow: "hidden", display: "flex", flexDirection: "column" }}>
      <div style={{ position: "relative", aspectRatio: "4 / 3", background: "var(--surface-2)" }}>
        ...
        >
          {prof?.emoji} {prof?.label ?? s.profession}
        </span>
        {s.video_url && (
          <span
            className="badge"
      ...
      <div style={{ padding: "14px 16px 16px", display: "flex", flexDirection: "column", gap: 6, flex: 1 }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
          <strong style={{ fontSize: "1.05rem", display: "inline-flex", alignItems: "center", gap: 5 }}>
            {s.name}
            {s.verified && <span title="Проверенный специалист" style={{ color: "#2a5bd7", fontSize: 13 }}>✔</span>}
          </strong>
        ...
        {s.tags.length > 0 && (
          <div style={{ display: "flex", flexWrap: "wrap", gap: 5, marginTop: 4 }}>
            {s.tags.slice(0, 3).map((t) => (
              <span key={t} style={{ fontSize: "0.72rem", color: "var(--text-mute)", background: "var(--surface-2)", padding: "2px 8px", borderRadius: 999 }}>
                #{t}
              </span>
            ))}
          </div>
        ...
        <div style={{ marginTop: "auto", paddingTop: 10, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <strong style={{ color: "var(--brand)" }}>{priceLabel(s.price_from)}</strong>
          <span className="soft" style={{ fontSize: "0.85rem", fontWeight: 600 }}>Смотреть →</span>
        </div>
      </div>
    </Link>
  );
